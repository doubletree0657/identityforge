package io.github.doubletree.iam.applications.application;
...
public class ClientApplicationService {
    ...
    public ClientSecretResult createClientWithSecret(
        ...
        if (tenant.getStatus() != TenantStatus.ACTIVE) {
            throw new ClientValidationException("Clients cannot be created in an inactive tenant");
        }
        if (clientRepository.existsByClientId(clientId)) {
            throw new ClientValidationException("OAuth2 client_id is already registered: " + clientId);
        }
    ...
    private ResourceServer loadResourceServerForTenant(UUID resourceServerId, UUID tenantId) {
        ...
        if (resourceServer.getStatus() != ResourceServerStatus.ACTIVE) {
            throw new ClientValidationException("Client cannot use an inactive resource server");
        }
