...
import io.github.doubletree.iam.provisioning.web.dto.ScimCreateGroupRequest;
import io.github.doubletree.iam.provisioning.web.dto.ScimCreateUserRequest;
...
@RestController
@RequestMapping("/scim/v2/{tenantId}")
...
public class ScimController {
    ...
    public ScimUserResponse createUser(
            ...
            @Valid @RequestBody ScimCreateUserRequest request) {
        User user = userApplicationService.createUser(tenantId, request.userName(), request.displayName());
        return ScimUserResponse.from(user);
    }

    @GetMapping("/Users/{id}")
    @Operation(summary = "Get SCIM user", description = "Requires iam.read scope.")
    public ScimUserResponse getUser(@PathVariable UUID tenantId, @PathVariable UUID id) {
        return ScimUserResponse.from(userApplicationService.findUser(tenantId, id));
    }
    ...
    public ScimGroupResponse createGroup(
            @PathVariable UUID tenantId,
            @Valid @RequestBody ScimCreateGroupRequest request) {
        Group group = groupApplicationService.createGroupWithMembers(
                tenantId, request.displayName(), request.members());
        return ScimGroupResponse.from(group);
    }

    @GetMapping("/Groups/{id}")
    @Operation(summary = "Get SCIM group", description = "Requires iam.read scope.")
    public ScimGroupResponse getGroup(@PathVariable UUID tenantId, @PathVariable UUID id) {
        return ScimGroupResponse.from(groupApplicationService.findGroup(tenantId, id));
    }
}
