...
class OAuth2AuthorizationCodeLoginFlowTests {
    ...
    void persistedConfidentialClientCompletesAuthorizationCodeLoginFlowAndAccessesProtectedApi()
        ...
        AuthenticatedSession authenticatedSession =
                login(startedAuthorization.session(), loginIdentifier(fixture.user()));
        ...
        assertThat(readJwt.getSubject()).isEqualTo(fixture.user().getId().toString());
        assertThat(readJwt.getClaimAsString("preferred_username")).isEqualTo(fixture.user().getUsername());
        assertThat(readJwt.getAudience()).containsExactly("identityforge-admin-api");
    ...
    void consentPageApprovalAndDenialUseOAuth2ErrorBehaviorAndAuditEvents() throws Exception {
        ...
        AuthenticatedSession approvalSession = login(approvalStart.session(), loginIdentifier(fixture.user()));
        ...
        AuthenticatedSession denialSession = login(denialStart.session(), loginIdentifier(denialFixture.user()));
    ...
    void authorizationCodeFlowAllowsAssignedApplicationPermissionScope() throws Exception {
        ...
        ResourceServer resourceServer = resourceServerApplicationService.createResourceServer(
                fixture.client().client().getTenant().getId(), "payroll-api", "OAuth Payroll API", null);
    ...
    private String loginIdentifier(User user) {
        return user.getTenant().getSlug() + "/" + user.getUsername();
    }
